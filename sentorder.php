...
if(!preg_match('/^[a-zA-Z]+$/',$oid) || empty($oid) || preg_match( '/[\'\/~`\!@#\$%\^&\*\(\)_\-\+=\{\}\[\]\|;:"\<\>,\.\?\\\]/' , $dname) || preg_match( '/[\s]+/',$dname)){
			array_push($errors,"order Id must be character!!");
...
if(!preg_match('/^[0-9]+$/',$sell) || empty($sell) || preg_match( '/[\'\/~`\!@#\$%\^&\*\(\)_\\+=\{\}\[\]\|;:"\<\>,\.\?\\\]/' , $sell) || preg_match( '/[\s]+/',$sell)){
 ...
 $date=date("Y-m-d");
	...
	if($drugname != $dname)	{
	//if($expire > $date){
	$sql = "insert into order(OrderId,drugName,DrugType,quantity,OrderDate,user_id) values 
	('$oid','$dname','$dtype','$quantity','$od','$userid')";
...
<div id="sse2">
       <div id="sses2">
         ...
         <ul id="unique">
		 <li><p style="margin-left:260px;font: bold 15px times new roman;">Well Come To Manager Page<p></li>
...
<strong><font color="white" size="4px"font-family="times new romman"; ><center> Drugs Order Information</center></font></strong>
 </div>
  <table width="286" valign="top" align="center" border="0">
  <tr>
	  <td class='para1_text' width="220px"><b>Order Id</b></td><td><input type="text" name="oid" id="password" required x-moz-errormessage="Enter password" maxlength="15"></td>
</tr>
<tr>
...
<!--<tr>
	 <td class='para1_text'><b> Manu date:</b></td><td><input type="date"name="manu"  >
</tr>
<tr>

	       <td class='para1_text' width="220px"> <b>Expire date:</b></td><td>
		   <input type="date"   name="expire"   required x-moz-errormessage="Enter phone number" ></td>
	...
	<tr>
		 ...
		 <!--<tr>
	       <td class='para1_text'><b> Unit Cost:</b></td><td><input type="text" name="unitcost" id="password" required x-moz-errormessage="Enter password" maxlength="15"></td>
	     </tr>
	<tr>

	       <td class='para1_text' width="220px"> <b>Selling Price</b></td><td>
		   <input type="text"   name="sell"   required x-moz-errormessage="Enter password" ></td>
		 
	     </tr>-->
	<tr>
		<td class='para1_text'>Ordered date:</td><td><input type="date" name="orddate"value="<?php $date=date("Y-m-d");echo $date;?>"  >
	</tr>
